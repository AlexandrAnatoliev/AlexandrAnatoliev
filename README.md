...
* test - программы для тестирования элементов оборудования
* courseBalakirev - курс "Язык С для начинающих" Сергея Балакирева
* eulersC - задачи Эйлера на С
...
## freelanceTask8
#### Описание: N работ могут исполняться n исполнителями. Известно время исполнения каждой работы каждым исполнителем tij,ij=1..n. Распределить работы между исполнителями так, чтобы каждая работа выполнялась одним исполнителем, каждый исполнитель выполнял одну работу, а суммарное время выполнения всех работ было минимальным.
...
#### Описание: Бот получает список рецептов из файла, создает словарь и рецепты по очереди (с рекламой!!!) и с фото блюда через случайные период времени постит в канал.

## blink_test
#### тест ардуино - мигает светодиодом 

## tableLamp
#### включаем и выключаем яркий белый свет по короткому нажатию сенсорной кнопки
#### изменяет яркость лампы длительным нажатием - изначально - уменьшаем яркость

## courseBalakirev
#### курс по языку Си от Сергея Балакирева
